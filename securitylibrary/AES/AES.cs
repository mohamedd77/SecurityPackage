...
namespace SecurityLibrary.AES
{
    ...
    public class AES : CryptographicTechnique
    {
        ...
        static byte[,] ISBOX = new byte[16, 16] {{ 0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb },
        ...
        byte[,] IgaloisField = new byte[4, 4] {{0x0e, 0x0b, 0x0d, 0x09},
                                                     {0x09, 0x0e, 0x0b, 0x0d},
                                                     {0x0d, 0x09, 0x0e, 0x0b},
                                                     {0x0b, 0x0d, 0x09, 0x0e}};
        int RCINDEX = 0;
        ...
        string ConvertMatrixToString(byte[,] matrix)
        {
            ...
            do
            {
                ...
                do
                {
                    var cell = Convert.ToString(matrix[y, x], 16);
                    if (cell.Length < 2)
                        str.Append("0" + cell);
                    else
                        str.Append(cell);
        ...
        byte[,] MakeStateByteMatrix(string str)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    Mat[y, x] = Convert.ToByte(cell, 16);
                    k += 2;
                    y++;
                } while (y < 4);
                x++;
        ...
        byte[,] MakeKeyByteMatrix(string str)
        {
            ...
            do
            {
                ...
                {
                    string cell = "0x" + str[k] + str[k + 1];
                    Mat[x, y] = Convert.ToByte(cell, 16);
                    k += 2;
                    y++;
                } while (y < 4);
                x++;
        ...
        byte[] SubByte(byte[] word)
        {
            ...
            do
            {
                string cell = Convert.ToString(word[x], 16);
                if (cell.Length == 1)
                {
                    newx = 0;
                    newy = Convert.ToInt32(cell[0].ToString(), 16);
                }
                else
                {
                    newx = Convert.ToInt32(cell[0].ToString(), 16);
                    newy = Convert.ToInt32(cell[1].ToString(), 16);
                }
                arr[x] = SBOX[newx, newy];
                x++;
            } while (x < 4);
        ...
        byte[] Xor(byte[] first, byte[] second, byte[] third, int isMultipleOf4)
        {
            ...
            do
            {
                string tmp;
                if (isMultipleOf4 == 0)
                    tmp = Convert.ToString(first[x] ^ second[x], 16);
                else
                    tmp = Convert.ToString(first[x] ^ second[x] ^ third[x], 16);

                arr[x] = Convert.ToByte(tmp, 16);
                x++;
            } while (x < 4);

            return arr;
        }
        void WorkOnKey(string K)
        {
            byte[,] K_arr = new byte[4, 4];
            K_arr = MakeKeyByteMatrix(K);
            int x = 0;
            do
            {
                int y = 0;
        ...
        byte[,] GetKeyMatrix(int index)
        {
            ...
            do
            {
                ...
                do
                {
                    Mat[column, row] = keyExpansionMat[x, y];
                    column++;
                    y++;
                } while (y < 4);
                row++;
        ...
        byte[,] AddRoundKey(byte[,] matrix, int roundNum)
        {
            ...
            do
            {
                ...
                do
                {
                    cell = Convert.ToString(keyRound[x, j] ^ matrix[x, j], 16);
                    keyRound[x, j] = Convert.ToByte(cell, 16);
                    j++;
                } while (j < 4);
                x++;
            } while (x < 4);
        ...
        void ImplementKeyExpansion()
        {
            ...
            for (int x = 4; x < 44; x++)
            {
                for (int y = 0; y < 4; y++)
                {
                    first[y] = keyExpansionMat[x - 1, y];
                    second[y] = keyExpansionMat[x - 4, y];
                    if (RCINDEX < 10)
                        third[y] = RCON[y, RCINDEX];
                }
                if (x % 4 == 0)
                {
                    RCINDEX++;
        ...
        void PrintKeyMatrix()
        {
            ...
            do
            {
                for (int y = 0; y < 44; y++)
                {
                    Console.Write(string.Join(", ", keyExpansionMat[y, x].ToString("X2")));
                    Console.Write(" ");
                }
                Console.WriteLine();
                x++;
        ...
        void PrintMatrix(byte[,] mat)
        {
            ...
            do
            {
                for (int y = 0; y < 4; y++)
                {
                    Console.Write(string.Join(", ", mat[x, y].ToString("X2")));
                    Console.Write(" ");
                }
                Console.WriteLine();
                x++;
        ...
        byte[,] MixCols(byte[,] shiftedMatrix)
        {
            ...
            do
            {
                ...
                do
                {
                    for (int z = 0; z < 4; z++)
                    {
                        if (galoisField[y, z] == 1)
                        {
                            XOR_arr[z] = shiftedMatrix[z, x];
                        }
                        else if (galoisField[y, z] == 2)
                        {
                            XOR_arr[z] = MultiplyByTwo(shiftedMatrix[z, x]);
                        }
                        else if (galoisField[y, z] == 3)
                        {
                            XOR_arr[z] = MltiplyByThree(shiftedMatrix[z, x]);
                        }
                    }
                    var cell = XOR_arr[0] ^ XOR_arr[1] ^ XOR_arr[2] ^ XOR_arr[3];
                    MixColsMat[y, x] = Convert.ToByte(cell);
                    y++;
                } while (y < 4);
                x++;
            } while (x < 4);
        ...
        byte MultiplyByTwo(byte x)
        {
            ...
            answer = (byte)(tmp & 0xFF);
            if (x > 127)
                answer = Convert.ToByte(answer ^ 27);
            return answer;
        }
        ...
        byte[,] MixColsInverse(byte[,] shiftedmatrix)
        {
            ...
            do
            {
                ...
                do
                {
                    for (int z = 0; z < 4; z++)
                    {
                        if (IgaloisField[y, z] == 0x9)
                        {
                            byte x0 = shiftedmatrix[z, x];
                            byte x1 = MultiplyByTwo(x0);
                            byte x2 = MultiplyByTwo(x1);
                            byte x3 = MultiplyByTwo(x2);
                            Xor_arr[z] = Convert.ToByte(x3 ^ x0);
                        }
                        if (IgaloisField[y, z] == 0xB)
                        {
                            byte x0 = shiftedmatrix[z, x];
                            byte x1 = MultiplyByTwo(x0);
                            byte x2 = MultiplyByTwo(x1);
                            byte x3 = MultiplyByTwo(x2);
                            Xor_arr[z] = Convert.ToByte(x3 ^ x0 ^ x1);
                        }
                        if (IgaloisField[y, z] == 0xD)
                        {
                            byte x0 = shiftedmatrix[z, x];
                            byte x1 = MultiplyByTwo(x0);
                            byte x2 = MultiplyByTwo(x1);
                            byte x3 = MultiplyByTwo(x2);
                            Xor_arr[z] = Convert.ToByte(x3 ^ x2 ^ x0);
                        }
                        if (IgaloisField[y, z] == 0xE)
                        {
                            byte x0 = shiftedmatrix[z, x];
                            byte x1 = MultiplyByTwo(x0);
                            byte x2 = MultiplyByTwo(x1);
                            byte x3 = MultiplyByTwo(x2);
                            Xor_arr[z] = Convert.ToByte(x3 ^ x2 ^ x1);
                        }
                    }
                    var cell = Xor_arr[0] ^ Xor_arr[1] ^ Xor_arr[2] ^ Xor_arr[3];
                    MixColMat[y, x] = Convert.ToByte(cell);
                    y++;
                } while (y < 4);
                x++;
            } while (x < 4);
        ...
        byte[,] DoInitialRound(byte[,] state)
        {
            ...
            do
            {
                ...
                do
                {
                    cell = Convert.ToString(state[y, x] ^ keyExpansionMat[x, y], 16);
                    state[y, x] = Convert.ToByte(cell, 16);
                    y++;
                } while (y < 4);
                x++;
            } while (x < 4);
        ...
        byte[,] SubBytes(byte[,] matrix)
        {
            ...
            do
            {
                for (int y = 0; y < 4; y++)
                {
                    string cell = Convert.ToString(matrix[x, y], 16);
                    int newI, newJ;
                    if (cell.Length == 1)
                    {
                        newI = 0;
                    ...
                    resultMatrix[x, y] = SBOX[newI, newJ];
                }
                x++;
            } while (x < 4);
        ...
        byte[,] ShiftRows(byte[,] matrix)
        {
            ...
            do
            {
                ...
                row = ShiftOneRow(row, x);
                for (int y = 0; y < 4; y++)
                {
                    resultMatrix[x, y] = row[y];
                }
                x++;
            } while (x < 4);
            return resultMatrix;
        ...
        byte[] ShiftRowInverse(byte[] row, int n)
        {
            ...
            do
            {
                number += Convert.ToUInt32(row[x]);
                if (x != 3) number = number << 8;
                x++;
            } while (x < 4);
